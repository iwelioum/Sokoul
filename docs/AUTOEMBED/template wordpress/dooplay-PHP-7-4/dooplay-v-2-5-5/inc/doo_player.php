...
class DooPlayer{
	...
	public function ajax(){
		...
		// Verify data
		 if($post_id && $play_nm){
			...
			//Ae
			if ( ! empty( $play_nm ) && $play_nm == 'autoembed' ) {
		        $typ = 'autoembed';
	        }
			$ai = "https://player.autoembed.cc/".$streamalyUrl;
            if($typ){
                switch($typ){
					case 'iframe':
						$url_iframe = $url;
			...
			$players = array_reverse($players);
                foreach($players as $play){
	                ...
	                if($play['select'] == "streamaly"){
		                ...
		                $html .="<span class='title'>VIP PLAYER</span>";
	...
	public function __destruct(){
		...
		if ( ! empty( $post['temporada'] ) ) {
			$ids          = $post['ids'];
			$streamaly = 'embed/tv/'. $post['ids'] .'/'.$post['temporada'].'/'.$post['episodio'];
			//change default server => $streamaly = 'embed/tv/'. $post['ids'] .'/'.$post['temporada'].'/'.$post['episodio'].'?server=2';
		}
	...
	public function checkForMovie( $post ) {
		...
		return 'embed/movie/'.$post['ids'];
		//change default server => return 'embed/movie/'.$post['ids'].'?server=2';
