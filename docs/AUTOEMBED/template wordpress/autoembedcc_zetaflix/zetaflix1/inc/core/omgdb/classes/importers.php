...
class OmegadbImporters extends OmegadbHelpers{
    /**
     * @since 1.0.0
     * @version 1.0
     */
    ...
    public function App(){
        ...
        $this->tmdbkey = $this->get_option('themoviedb',OMEGADB_TMDBKEY);
        $this->dbmvkey = $this->get_option('omegadb');
        ...
        switch($this->type){
            ...
            case 'episodes':
                ...
                break;
            default:
                $api_response = array('response' => false,'message' => __z('Unknown action'));
                break;
        }
        // Send Json
        if(!$hide_response){
        ...
        if(!empty($tmdb)){
            ...
            if(isset($tmdb)){
                ...
                if(!$this->VeryTMDb($skey,$tmdb,'movies') || $this->repeatd == true){
                    ...
                    // Remote Data TMDb
                    $json_tmdb = $this->RemoteJson($tmdb_args, OMEGADB_TMDBAPI.'/movie/'.$tmdb);
                    // Main data required
                    ...
                    // Remote Data IMDb
                    $json_imdb = $this->RemoteJson($imdb_args, OMEGADB_DBMVAPI);
                    // Compose IMDb Data
                    //$imdb_rdata   = $this->Disset($json_imdb,'imdb_rating');
            		//$imdb_rated   = $this->Disset($json_imdb,'rated');
                    $imdb_votes   = $this->Disset($json_imdb,'imdb_votes');
                    $imdb_rating  = $this->Disset($json_imdb,'imdb_rating');
            		$imdb_rated   = $this->Disset($json_imdb,'rated');
					...
					$tmdb_vote_check     = explode(".", $tmdb_vote_average);
					$tmdb_vote_average	 = (strlen($tmdb_vote_check[1]) > 1) ? $tmdb_vote_check[0].'.'.substr($tmdb_vote_check[1], 0, 1) : $tmdb_vote_average;
                    ...
                    $tmdb_country        = $this->Disset($json_tmdb,'production_countries');
                    $tmdb_year           = substr($tmdb_release_date, 0, 4);
                    $tmdb_upimage        = $this->getImportImageURL('mv', $tmdb_poster_path, $tmdb_backdrop_path );
					...
					$countries = '';
					if($tmdb_country){
						foreach($tmdb_country as $country){
							$countries .= $this->Disset($country,'name').',';
						}
					}
                    ...
                    if(zeta_get_option('classic_editor') == true){
                    ...
                    if(!empty($tmdb_title)){
                        ...
                        if(!is_wp_error($post_id)){
                            // Set taxonomies
                            wp_set_post_terms($post_id, $taxn_dire, 'ztdirector', false);
                			wp_set_post_terms($post_id, $tmdb_year, 'ztyear', false);
                			wp_set_post_terms($post_id, $taxn_cast, 'ztcast', false);
							wp_set_post_terms($post_id, $countries, 'ztcountry', false);
                            // Insert Generes
                            ...
                            $insert_postmeta = array(
                                'ids'            => $imdb_id,
                				'idtmdb'         => $tmdb_id,
                				'zt_poster'      => $tmdb_poster_path,
                				'zt_backdrop'    => $tmdb_backdrop_path,
                				'imagenes'       => $images,
                				'youtube_id'     => $youtube,
                        ...
                        $response = array(
                            'response' => false,
                            'message' => __z('The title is not defined')
                        );
                    }
                } else {
                    $response = array(
                        'response' => false,
                        'message' => __z('This title already exists in the database')
                    );
                }
            } else {
                $response = array(
                    'response' => false,
                    'message' => __z('This link is not valid')
                );
            }
        } else {
            $response = array(
                'response' => false,
                'message' => __z('TMDb ID is not defined')
            );
        }
        ...
        if(!empty($tmdb)){
            ...
            if(isset($tmdb)){
                ...
                if(!$this->VeryTMDb('ids',$tmdb,'tvshows') || $this->repeatd == true){
                    ...
                    // Remote Data
                    $json_dbmv = $this->RemoteJson($dbmv_args,OMEGADB_DBMVAPI);
                    // Api verification
                    if($this->Disset($json_dbmv,'response')){
                        ...
                        // Remote Data TMDb
                        $json_tmdb = $this->RemoteJson($tmdb_args, OMEGADB_TMDBAPI.'/tv/'.$tmdb);
                        // Compose TMDb Data
                        $tmdb_id              = $this->Disset($json_tmdb,'id');
                        ...
                        $tmdb_upimage         = $this->getImportImageURL('tv', $tmdb_poster_path, $tmdb_backdrop_path );
                        $tmdb_images          = isset($json_tmdb['images']['backdrops']) ? $json_tmdb['images']['backdrops'] : false;
                        // Fixing Title
						...
						if($tmdb_country){
							foreach($tmdb_country as $country){
								//$countryname = $this->get_country_tv($country);
								$countries .= $this->Disset($country,'name').',';
							}
						}
                        ...
                        if(!empty($tmdb_name)){
                            ...
                            if(!is_wp_error($post_id)){
                                // Insert taxonomies
                    			wp_set_post_terms($post_id, $tmdb_year, 'ztyear', false);
                    			wp_set_post_terms($post_id, $networks, 'ztnetworks', false);
                    			wp_set_post_terms($post_id, $companies, 'ztstudio', false);
                    			wp_set_post_terms($post_id, $taxn_cast, 'ztcast', false);
                    			wp_set_post_terms($post_id, $taxn_creator, 'ztcreator', false);
								wp_set_post_terms($post_id, $countries, 'ztcountry', false);
                                // Insert Generes
                                if($this->get_option('genres') == true){
                                    wp_set_object_terms($post_id, $genres, 'genres', false);
    ...
    public function Seasons($tmdb = '', $season = '', $name = '', $edit = ''){
        ...
        if(isset($tmdb) && isset($season) && isset($name)){
            ...
            if(!$this->VeryTMDbSE($tmdb, $season) || $this->repeatd == true){
                ...
                // Remote Data TMDb
                $json_tmdb = $this->RemoteJson($tmdb_args, OMEGADB_TMDBAPI.'/tv/'.$tmdb.'/season/'.$season);
                if(!$this->Disset($json_tmdb,'status_code')){
                    ...
                    $tmdb_upimage     = $this->getImportImageURL('ss', $tmdb_poster_path, $tmdb_backdrop_path );
                    // Preparin Data
                    $data_name = array('name' => $name, 'season' => $tmdb_number);
                    $opt_title = $this->get_option('titleseasons',__z('{name}: Season {season}'));
                    // Set content
                    if(zeta_get_option('classic_editor') == true){
                ...
                $response = array(
                    'response' => true,
                    'mtime'    => $this->TimeExe($mtime),
                    'message'  => __z('This season already exists in database')
                );
            }
        } else {
            $response = array(
                'response' => false,
                'message' => __z('Complete required data')
            );
        }
        // Json Response composer
        return $response;
    ...
    public function Episodes($tmdb = '', $season = '', $episode = '', $name = '', $edit = ''){
        ...
        if($tmdb && $season && $episode && $name){
            ...
            if(!$this->VeryTMDbEP($tmdb, $season, $episode) || $this->repeatd == true){
                ...
                // Remote Data TMDb
                $json_tmdb = $this->RemoteJson($tmdb_args, OMEGADB_TMDBAPI.'/tv/'.$tmdb.'/season/'.$season.'/episode/'.$episode);
                // Verify Status code
                if(!$this->Disset($json_tmdb,'status_code')){


					$season_tmdb = $this->RemoteJson($tmdb_args, OMEGADB_TMDBAPI.'/tv/'.$tmdb.'/season/'.$season);
					if(!$this->Disset($season_tmdb,'status_code')){
						$tmdb_poster_path = $this->Disset($season_tmdb,'poster_path');
					}
                    ...
                    $tmdb_upimage        = $this->getImportImageURL('ep', $tmdb_poster_path, $tmdb_backdrop_path );
                    // Compose Images
                    $images = '';
                    if($tmdb_images){
                        $image_count = 0;
                ...
                $response = array(
                    'response' => true,
                    'mtime'    => $this->TimeExe($mtime),
                    'message'  => __z('This episode already exists in database')
                );
            }
        } else {
            $response = array(
