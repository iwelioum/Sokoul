...
class OmegadbUpdater extends OmegadbHelpers{
    ...
    */
    public function __construct(){
        ...
        // Ajax Action
        add_action('wp_ajax_omegadb_metaupdater', array($this,'ajax_action'));
        ...
        switch ($this->get_option('updatermethod')) {
            ...
            break;
            case 'wp-cron':
                if(get_option('__omgdb_cronmeta_status') == 'processing'){
                    if(!wp_next_scheduled('omegadb_cron_metaupdater')){
                        wp_schedule_event(time(),'every_1_minute','omegadb_cron_metaupdater');
                    }
                    add_action('omegadb_cron_metaupdater',array($this,'cron_action'));
                }
            break;
        }
        ***/
    ...
    */
    public function cron_action(){
        update_option('cron_omegadb_timer', time());
    }
    ...
    public function ajax_action(){
        ...
        if($_SERVER['REQUEST_METHOD'] === 'POST'){
            ...
            switch ($action) {
                // Updating Metada
                case 'updater':
                    $page     = get_option('__omgdb_cronmeta_paged','1');
                    ...
                    if($content){
                        ...
                        // Update Options
                        delete_option('__omgdb_cronmeta_total');
                        delete_option('__omgdb_cronmeta_pages');
                        delete_option('__omgdb_cronmeta_paged');
                        // The Response
                        $response = array(
                            'success' => false,
                            'message' => __z('There is no content to update')
                        );
                    }
                break;
                ...
                case 'finish':
                    ...
                    $response = array(
                        'success' => false,
                        'message' => __z('Process finished, the metadata update was incomplete')
                    );
                    break;
            }
        }
    ...
    */
    private function calculate_content(){
        $total = get_option('__omgdb_cronmeta_total');
        if(!$total){
            $query = new WP_Query(
                array(
            ...
            update_option('__omgdb_cronmeta_total',$total);
        }
        return $total;
    }
    ...
    */
    private function calculate_pages($per_page = 1){
        $pages = get_option('__omgdb_cronmeta_pages');
        // Compose Query
        if(!$pages){
            $pages = ceil($this->calculate_content()/$per_page);
            // Update pages
            update_option('__omgdb_cronmeta_pages',$pages);
        }
        return $pages;
    }
    ...
    */
    private function get_content($post_id, $post_type, $page){
        ...
        $this->omgdb_key = $this->get_option('omegadb');
        $this->tmdb_key  = $this->get_option('themoviedb',OMEGADB_TMDBKEY);
        ...
        if(!empty($post_id) && !empty($post_type)){
            ...
            $response = array(
                'success' => false,
                'message' => __z('Incomplete data')
            );
        }
        // Recount page
        update_option('__omgdb_cronmeta_paged', $page+1);
        // The Response
        return $response;
    }
    ...
    */
    private function movies($post_id = ''){
        ...
        if(!empty($finder)){
            // TMDb Data
            $json_tmdb = $this->RemoteJson($this->arguments('images,trailers,credits'),OMEGADB_TMDBAPI.'/movie/'.$finder);
            // verificate
            if(!$this->Disset($json_tmdb,'status_code')){
				...
				$average   = (strlen($average[1]) > 1) ? $average[0].'.'.substr($average[1], 0, 1) : $average;
                ...
                // Set IMDb Data
                $json_imdb = $this->RemoteJson(array('key'=>$this->omgdb_key,'imdb'=>$imdb_id), OMEGADB_DBMVAPI);
                // Compose IMDb Data
                ...
                ################################ UPDATE POST META ##################################
                if(!empty($poster))
                    update_post_meta($post_id,'zt_poster',sanitize_text_field($poster));
                if(!empty($backdrop))
                    update_post_meta($post_id,'zt_backdrop',sanitize_text_field($backdrop));
                if(!empty($images))
                ...
                if(!empty($meta_cast))
                    update_post_meta($post_id,'zt_cast',sanitize_text_field($meta_cast));
                if(!empty($meta_director))
                    update_post_meta($post_id,'zt_dir',sanitize_text_field($meta_director));
                ...
                #####################################################################################
                $response = $this->successfully($post_id, __z('movie'));#############################
                #####################################################################################
            }else{
                $response = array(
                    'success' => false,
    ...
    */
    private function tvshows($post_id = ''){
        ...
        if(!empty($finder)){
            // TMDb Json data
            $json_tmdb = $this->RemoteJson($this->arguments('images,videos,credits'),OMEGADB_TMDBAPI.'/tv/'.$finder);
            // verificate
            if(!$this->Disset($json_tmdb,'status_code')){
                // Get videos
                ...
                if(!empty($meta_creator))
                    update_post_meta($post_id,'zt_creator',sanitize_text_field($meta_creator));
                // Upload Image
                if(!empty($poster) && !has_post_thumbnail($post_id))
                    $this->UploadImage('https://image.tmdb.org/t/p/w780'.$poster, $post_id, true, false);
                #####################################################################################
                $response = $this->successfully($post_id, __z('show'));##############################
                #####################################################################################
            } else {
                $response = array(
                    'success' => false,
    ...
    */
    private function seasons($post_id = ''){
        ...
        if(!empty($finder) && !empty($season)){
            // TMDb Json data
            $json_tmdb = $this->RemoteJson($this->arguments(),OMEGADB_TMDBAPI.'/tv/'.$finder.'/season/'.$season);
            // verificate
            if(!$this->Disset($json_tmdb,'status_code')){
                // get Data
                $poster  = $this->Disset($json_tmdb,'poster_path');
                ...
                if(!empty($poster) && !has_post_thumbnail($post_id))
                    $this->UploadImage('https://image.tmdb.org/t/p/w780'.$poster, $post_id, true, false);
                #####################################################################################
                $response = $this->successfully($post_id,__z('season'));#############################
                #####################################################################################
            }else{
                $response = array(
                    'success' => false,
    ...
    */
    private function episodes($post_id = ''){
        ...
        if(!empty($finder) && !empty($season) && !empty($episode)){
            // TMDb Json data
            $json_tmdb = $this->RemoteJson($this->arguments(),OMEGADB_TMDBAPI.'/tv/'.$finder.'/season/'.$season.'/episode/'.$episode);
            // verificate
            if(!$this->Disset($json_tmdb,'status_code')){
                $airdate      = $this->Disset($json_tmdb,'air_date');
                ...
                #####################################################################################
                $response = $this->successfully($post_id, __z('episode'));############################
                #####################################################################################
            }else{
                $response = array(
                    'success' => false,
