...
function starstruck_action_links( $links ) {
	$action_links = array(
		'settings' => '<a href="' . admin_url('options-general.php?page=stars-rating') . '" title="' . esc_attr( __z('View Starstruck Settings') ) . '">' . __z('Settings') . '</a>',
	);
...
function starstruck_load_scripts() {
	...
	if (is_singular()) {
		wp_enqueue_script('jquery-raty', get_template_directory_uri(). '/assets/js/lib/starstruck.raty.js', false, ZETA_VERSION );
		wp_enqueue_script('starstruck-js', get_template_directory_uri(). '/assets/js/lib/starstruck.js', array('jquery-raty'), ZETA_VERSION );
...
function starstruck_return_content( $data, $content, $id, $type ) {
	...
	if ( $data = maybe_unserialize( $data ) ) {
		$rating = zeta_isset($data,'avg');
		$votes  = zeta_isset($data,'votes');
		$user_info = starstruck_user_object();
		if(is_array(zeta_isset($data,'users')) && array_key_exists( $user_info->ID, zeta_isset($data,'users'))) {
...
function starstruck_return_content_span( $id, $rating, $type, $class = 'starstruck-main') {
	return '<div class="rating-stars starstruck ' . esc_attr( $class ) .' " data-id="' . esc_attr($id) . '" data-rating="' .esc_attr( $rating ) . '" data-type="' .esc_attr( $type ) . '"></div>';
}
...
function starstruck_action_callback() {
	if ( ! wp_verify_nonce( $_POST['nonce'], 'starstruck-nonce') ) {
		header( __z('HTTP/1.1 400 Nonce is empty and/or invalid') );
	}
	if ( ! isset( $_POST['id'] ) ) {
		header( __z('HTTP/1.1 400 ID is empty') );
	} else {
		$id = $_POST['id'];
	}
	if ( ! isset( $_POST['type'] ) ) {
...
function starstruck_delete_action_callback() {
	...
	$response = array('text' => __z('User vote deleted') );
	header('Content-Type: application/json');
	echo json_encode( $response );
	die();
}
...
function starstruck_user_object() {
	...
	if ( $user_ID > 0 ) {
		...
		$user_info->display_name = __z('Anonymous');
	}
	return $user_info;
}
		...
		?>
				<li>
					<div class="starstruck-wrap">
						...
						<div class="starstruck-rating"><?php _z('Rating:'); ?> <?php echo $rating; ?> - <?php echo number_format( $votes ); ?> <?php echo _n('vote', 'votes', $votes ); ?></div>
					</div><!-- .starstruck-wrap -->
				</li>
		<?php
			$index++;
