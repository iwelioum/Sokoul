...
$itmdb = zeta_isset($postmeta,'ids');
$seaso = zeta_isset($postmeta,'temporada');
	...
	if($query && is_array($query)){
		foreach($query as $post_id){
			...
			$html_out .= "<li class='ep-{$episo} {$active}'>
							<a class='play-ep' data-sec='".$nonce."' data-type='ssep' data-pid='{$post_id}' data-epid='{$episo}' data-ssid='{$seaso}' {$url}>
