...
$out = "<tr id='link-row-{$psid}' link-id='{$psid}'>";
...
$out .= "<li><a href='#' class='edit_link chng' data-id='{$psid}'>".__z('Edit')."</a></li>";
if(current_user_can('administrator')){
    if($stus == 'publish'){
        $out .= "<li><a href='#' class='control_link updt' data-user='{$usid}' data-id='{$psid}' data-status='pending'>".__z('Disable')."</a></li>";
    } else {
        $out .= "<li><a href='#' class='control_link updt' data-user='{$usid}' data-id='{$psid}' data-status='publish'>".__z('Enable')."</a></li>";
    }
	$visibility = ($stus == 'trash') ? "style='display:none'" : "";
    $out .= "<li {$visibility}><a href='#' class='control_link dlt' data-user='{$usid}' data-id='{$psid}' data-status='trash'>".__z('Delete')."</a></li>";
