...
$items = zeta_get_option('itopimdb','50');
...
$page_topmovies = get_transient('zetaflix_page_topmovies');

$page_toptvshow = get_transient('zetaflix_page_toptvshow');
...
if(false === $page_topmovies){
    ...
    set_transient('zetaflix_page_topmovies',$page_topmovies, 1 * HOUR_IN_SECONDS);
...
if(false === $page_toptvshow){
    ...
    set_transient('zetaflix_page_toptvshow',$page_toptvshow, 1 * HOUR_IN_SECONDS);
	...
	case 'movtv':
        ...
        if($page_topmovies){
            ...
            foreach($page_topmovies as $key => $post_id) {
                ...
                zeta_topimdb_item($num, $post_id, 'movies');
        ...
        if($page_toptvshow){
            ...
            foreach($page_toptvshow as $key => $post_id) {
                ...
                zeta_topimdb_item($num, $post_id, 'tvshows', 'Vertical');
	...
	case 'movie':
        ...
        if($page_topmovies){
            ...
            foreach($page_topmovies as $key => $post_id) {
                ...
                zeta_topimdb_item($num, $post_id, 'movies', 'Vertical', 'zt_poster');
