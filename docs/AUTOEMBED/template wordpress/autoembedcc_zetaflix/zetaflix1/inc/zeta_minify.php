...
if( ! function_exists( 'zt_init_minify_html' ) ) {
	function zt_init_minify_html() {
		$minify_html_active = zeta_is_true('permits','mhtm');
		if ( $minify_html_active == true) ob_start('zt_minify_html_output');
	}
	if ( !is_admin() ) add_action('init', 'zt_init_minify_html', 1 );
}

if( ! function_exists( 'zt_minify_html_output' ) ) {
	function zt_minify_html_output($buffer) {
