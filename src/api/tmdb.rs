use crate::{cache::{get_from_cache, set_to_cache_with_ttl}, clients::tmdb::DiscoverParams, AppState, api::error::ApiError};
...
pub fn tmdb_routes() -> Router<Arc<AppState>> {
    Router::new()
        .route("/tmdb/trending/:media_type/:time_window", get(trending_handler))
        ...
        .route("/tmdb/tv/:tv_id/season/:season_number", get(season_details_handler))
        .route("/tmdb/:media_type/:id/credits", get(credits_handler))
        .route("/tmdb/:media_type/:id/videos", get(videos_handler))
        .route("/tmdb/:media_type/:id/watch/providers", get(watch_providers_handler))
        .route("/tmdb/person/:id", get(person_details_handler))
        .route("/tmdb/person/:id/credits", get(person_combined_credits_handler))
...
async fn handle_cache<T: Serialize + DeserializeOwned>(
    ...
    match fetcher.await {
        Ok(data) => {
            if let Err(e) = set_to_cache_with_ttl(&state.redis_client, &key, &data, ttl as u64).await {
...
pub async fn discover_handler(
    ...
    let key = format!("tmdb:discover:{}:{:?}", media_type, params);
    handle_cache(&state, key, 3600, async {
